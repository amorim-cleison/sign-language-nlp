...
for i in "${GPU[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES=$i poetry run dask-worker "${DASK_HOST}:${DASK_PORT}" --name "worker-gpu${i}" --nworkers 1 --nthreads ${THREADS} &> ~/repos/sign-language-nlp/cluster/out/az-worker-gpu${i}.out &
