...
def prepare_training(cuda_enabled, devices, train_data, val_data, TGT, SRC,
    ...
    # Optimizer:
    model_opt = get_model_opt(model_par=model_par, **training_args)
    ...
    # Restore states:
    last_epoch, model, model_opt = restore_states(checkpoint_dir, model_par,
                                                  model_opt)
...
def run_training(cuda_enabled, devices, epochs, train_iter, val_iter,
                 criterion, model, model_par, model_opt, loss_compute,
                 next_epoch, checkpoint_interval, checkpoint_dir,
                 log_steps_interval, pad_idx, SRC, TGT, config_path, **kwargs):
    # model_par = nn.DataParallel(model, device_ids=devices)
    ...
    for epoch_idx in range(next_epoch, epochs):
        ...
        if checkpoint_interval and ((epoch % checkpoint_interval) == 0 or
                                    (epoch == epochs)):
            save_checkpoint(checkpoint_dir, config_path, epoch, model_par,
                            model_opt, train_loss)
...
def save_checkpoint(dir, config_path, epoch, model_par, optimizer, loss):
    ...
    state = {
        'epoch': epoch,
        'model_state_dict': model_par.state_dict(),
...
def restore_states(dir, model_par, optimizer):
    weigths_path = normpath(f"{dir}/weights.tar")
    ...
    if exists(weigths_path):
        ...
        model_par.load_state_dict(checkpoint['model_state_dict'])
        # optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    else:
        last_epoch = None
    return last_epoch, model_par, optimizer
...
def get_model_opt(model_par, warm_up, lr, betas, eps, **kwargs):
    return NoamOpt(
        model_par.module.d_model, 1, warm_up,
