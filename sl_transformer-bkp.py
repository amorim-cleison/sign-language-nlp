...
def run(devices_args, dataset_args, model_args, training_args,
    ...
    log(f"cuda enabled: {CUDA_ENABLED}")
...
def get_iter(dataset, batch_size, cuda_enabled, devices, train):
    if dataset:
        return MyIterator(dataset,
                          batch_size=batch_size,
                          # device=get_device(cuda_enabled),
                          device=devices[0] if cuda_enabled else None,
                          repeat=False,
...
def prepare_training(cuda_enabled, devices, train_data, val_data, TGT, SRC,
    ...
    train_iter = get_iter(train_data, batch_size, cuda_enabled, devices, True)
    val_iter = get_iter(val_data, batch_size, cuda_enabled, devices, False)
...
def run_training(cuda_enabled, devices, epochs, train_iter, val_iter,
                 criterion, model, model_par, model_opt, loss_compute, next_epoch,
                 checkpoint_interval, checkpoint_dir, log_steps_interval,
                 pad_idx, SRC, TGT, config_path, **kwargs):
    model_par = nn.DataParallel(model, device_ids=devices, output_device=torch.device("cpu"))
    ...
    for epoch_idx in range(next_epoch, epochs):
        ...
        log_phase("Training")
        model_par.train()
        train_loss_compute = get_loss_compute(model_par.module.generator, criterion, model_opt, devices, cuda_enabled)

        train_loss = run_epoch(data_iter=train_iter,
                               model=model_par,
                               loss_compute=train_loss_compute,
                               pad_idx=pad_idx,
                               log_steps_interval=log_steps_interval)

        mp = list(model.parameters())
        mpp = list(model_par.parameters())
        lp = list(train_loss_compute.opt.optimizer.param_groups[0]['params'])
        ...
        if val_iter:
            ...
            with torch.no_grad():
                val_loss_compute = get_loss_compute(model_par.module.generator, criterion, None, devices, cuda_enabled)
                ...
                # Accuracy
                accuracy = 9.999
                # accuracy = calculate_accuracy(model_par, val_iter, pad_idx, SRC, TGT)
...
def calculate_accuracy(model, val_iter, pad_idx, SRC, TGT, **kwargs):
    ...
    for i, batch in enumerate(val_iter):
        with torch.no_grad():
            ...
            out = greedy_decode(model.module,
                                src,
                                src_mask,
                                max_len=60,
